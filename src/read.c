...
ssize_t fipc_read(int64_t _fd, void *buf, size_t size) {
  ...
  while (likely(size_left)) {
    ...
    if (likely(channel->backlog.amount)) {
      copy_size = channel->backlog.amount > size_left ? size_left
                                                      : channel->backlog.amount;
      memcpy((char *)buf + ret_size,
             channel->backlog.buf + channel->backlog.offset, copy_size);
    ...
    ret = get_op(fd.mgmt.control & FIPC_FD_MASK)->wait_rde(fd, channel);
    if (unlikely(ret < 0)) {
      channel->rd_idx--;
      if (ret_size == 0) ret_size = ret;
      goto done;
    ...
    copy_size = size_left > channel->blocks[idx].amount
                    ...
                    : size_left;
    memcpy((char *)buf + ret_size, channel->blocks[idx].buf, copy_size);
...
#endif
      memcpy(channel->backlog.buf, channel->blocks[idx].buf + copy_size,
             channel->blocks[idx].amount);
    ...
    size_left -= copy_size;
    ret_size += copy_size;
    ret = get_op(fd.mgmt.control & FIPC_FD_MASK)->notify_wte(fd, channel);
    if (ret <= 0) {
      // fatal error
      ret_size = -1;
      goto done;
