...
{
        return read(fd.mgmt.rde, pesudo_pipe_buffer, pipe_buffer_size / FIPC_BLOCK_NUMBER);
}

int pipe_wait_wte(fipc_fd fd, fipc_block *block)
{
        return write(fd.mgmt.wte, pesudo_pipe_buffer, pipe_buffer_size / FIPC_BLOCK_NUMBER);
}
