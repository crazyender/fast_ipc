...
int pipe_open(fipc_fd fds[2]) {
  ...
  if (configure_pipe_size(pipefd[0]) < 0) goto fail;

  if (configure_pipe_size(pipefd[1]) < 0) goto fail;
...
int pipe_close(fipc_fd fd) {
  int ret = 0;
  if (fd.mgmt.rde >= 0) ret = (close(fd.mgmt.rde) < 0);

  if (fd.mgmt.wte >= 0) ret |= (close(fd.mgmt.wte) < 0);
...
int pipe_wait_rde(fipc_fd fd, fipc_channel *channel) {
  ...
  fipc_block *block =
      &channel->blocks[(channel->rd_idx - 1) % FIPC_BLOCK_NUMBER];

  if (atomic_get(&block->status) != 0) return 1;
#ifdef DEBUG
  else
  ...
  if (pollfd.revents & errorevent) return -1;
...
int pipe_wait_wte(fipc_fd fd, fipc_channel *channel) {
  ...
  int errorevent = POLLHUP | POLLERR | POLLNVAL;
  fipc_block *block =
      &channel->blocks[(channel->wt_idx - 1) % FIPC_BLOCK_NUMBER];
