...
int fipc(int64_t fipcfd[2], fipc_type type) {
  ...
  snprintf(name, 64, "/shm_%d_%lld.fipc", getpid(), name_index);
  shm_fd = shm_open(name, O_RDWR | O_CREAT | O_EXCL, S_IRUSR | S_IWUSR);
  if (shm_fd < 0) goto fail;
  ...
  if ((events_fd[0].mgmt.control & FIPC_FD_MASK) == FIPC_FD_PIPE) {
...
fail:
  ...
  if (shm_fd2 >= 0) close(shm_fd2);

  for (i = 0; i < 2; i++) {
    if (events_fd[i].raw < 0) continue;
...
int fipc_close(int64_t _fd) {
  ...
  if (!(fd.mgmt.control & FIPC_FD_MASK)) return close((int)_fd);
...
int64_t fipc_dup(int64_t fd) {
  ...
  if (newfd.mgmt.shm < 0) goto fail;

  newfd.mgmt.wte = dup(oldfd.mgmt.wte);
  if (newfd.mgmt.wte < 0) goto fail;

  newfd.mgmt.rde = dup(oldfd.mgmt.rde);
  if (newfd.mgmt.rde < 0) goto fail;

  return newfd.raw;

fail:
  if (newfd.mgmt.shm >= 0) close(newfd.mgmt.shm);

  if (newfd.mgmt.wte >= 0) close(newfd.mgmt.wte);

  if (newfd.mgmt.rde >= 0) close(newfd.mgmt.rde);

  return -1;
}

void fipc_init() { fd_cache_init(); }
