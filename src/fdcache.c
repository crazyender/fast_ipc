...
void lock_fd_read(int fd) {
  if (fd < 0 || fd >= MAX_CACHE_FD) return;
...
fipc_channel *get_channel(int fd) {
  if (fd < 0 || fd >= MAX_CACHE_FD) return NULL;

  // fast path
  if (fds[fd] != NULL) return fds[fd];

  // slow path
  if (fcntl(fd, F_GETFL) < 0) return NULL;

  void *addr = mmap(NULL, sizeof(fipc_channel), PROT_READ | PROT_WRITE,
                    MAP_SHARED, fd, 0);
  if (addr == (void *)MAP_FAILED) return NULL;
