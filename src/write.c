...
ssize_t fipc_write(int64_t _fd, void *buf, size_t size) {
  ...
  while (likely(size_left)) {
    ...
    int ret = get_op(fd.mgmt.control & FIPC_FD_MASK)->wait_wte(fd, channel);
    if (unlikely(ret < 0)) {
      channel->wt_idx--;
