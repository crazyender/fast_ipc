...
static void read_and_send_file(const char *name, int64_t fd) {
  ...
  fipc_write(fd, &filesize,
             sizeof(filesize));  // we assume 8 bytes are not possible to fail..

  while (size_left > 0) {
    int64_t read_size =
        (size_left > SEND_BUFFER_SIZE) ? SEND_BUFFER_SIZE : size_left;
    ...
    while (buf_left > 0) {
      ...
      if (ret < 0) die("send fail, size_left %lld, fd %.16llx", size_left, fd);
      buf_left -= ret;
      size_left -= ret;
      tmp += ret;
    }
...
int main(int argc, char **argv) {
  fipc_init();
  if (argc < 2) die("usage: %s [read|write] [type] [fd]\n", argv[0]);
