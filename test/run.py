...
for thread in threads:
    for ipc in types.keys():
        ...
        for size in chunk_size:
            sys.stdout.write("\rrun at threads {0} type {1} (left {2} seconds) => ".format(
                thread, types[ipc], total_time))
            sys.stdout.flush()
            cmd = "LD_LIBRARY_PATH=\".\" ./perf.test threads={0} size={1} type={2} loops={3} nohead=1".format(
                thread, size, ipc, loops)
        ...
        f.write("{0},{1},{2},{3}\n".format(
            thread, max_in_size, types[ipc], max_val))
        sys.stdout.write("type:{0} at {1} value {2}\n".format(
            types[ipc], max_in_size, max_val))
        f.flush()
f.close()
