...
static void help(const char *exec_name) {
  ...
  log_info(
      "\t type=[1|2|3|4] (1:pipe;2:event_fd;3:spin at "
...
static void parse_arg(char *arg, const char *opt, int64_t *val) {
  if (strstr(arg, opt) == arg) *val = get_val_from_opt(arg, opt);
}
...
static int timer_callback_fun(int real_dur, void *p) {
  ...
  int64_t write_perf =
      real_dur ? (total_write * 1000 / (real_dur * 1024 * 1024)) : 0;

  printf("%ld,%d,%ld,%ld", total_mem / 1024, real_dur, read_perf, write_perf);
#ifdef DEBUG
  printf(",%ld,%ld,%ld,%ld", total.read_contents, total.write_contents,
         total.syscalls, total.backlog_used);
#endif
  printf("\n");
  test_times++;
